...
import com.blackheart.brightscreenstatistics.MyApplication;
...
public class RunningTaskUtil {
  ...
  public static String s="11111222";

  //获取权限
  public RunningTaskUtil(Context context) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      mUsageStatsManager = (UsageStatsManager) context.getApplicationContext().getSystemService(
          Context.USAGE_STATS_SERVICE);
    }
    activityManager = (ActivityManager) context.getApplicationContext()
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (isFirst) {
        stats = mUsageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY,
            time - TWENTYSECOND, time);
      } else {
        stats = mUsageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY,
            time - THIRTYSECOND, time);
      }
      ...
      if (runningTopActivity.getPackageName()
          .equals(MyApplication.getInstance().getPackageName())) {
        runningTopActivity = getTopActivtyFromLolipopOnwards();
      }
    } else {
      runningTopActivity = getTopActivtyFromLolipopOnwards();
